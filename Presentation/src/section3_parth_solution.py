...
class ParthSolution():
    ...
    def play_parth_solution(self):
        ...
        if isinstance(self.scene, VoiceoverScene):
            script1 = "With just 3 lines of code, Parth can be integrated into state-of-the-art \
                sparse Cholesky solvers, such as  Apple Accelerate, MKL Pardiso, and CHOLMOD"
            with self.scene.voiceover(text=script1) as tracker:
                self._run_scene()

            script2 = "Achieving up to 5.9× speedup per solves!"
            with self.scene.voiceover(text=script2) as tracker:
                pass

            script3 = "It provides this performance benefits using 3 steps. In first step,\
